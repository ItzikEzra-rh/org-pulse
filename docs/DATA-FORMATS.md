...
## System Health — Component Maturity (`data/system-health/component-maturity/maturity-report.json`)

Precomputed OSAC maturity data generated by `org-pulse-data`. The application
serves this file unchanged and performs no evidence collection or scoring.

The schema-version 1 report is a normalized document with these top-level
fields:

| Field | Type | Description |
|-------|------|-------------|
| `schemaVersion` | number | Contract version; currently `1` |
| `source` | object | Collector name, version, and optional CI URL |
| `generatedAt` | ISO string | Time of the successful evidence collection |
| `repositories` / `images` | array | Referenced source and image catalogs |
| `rules` | array | DP/TP/GA rule definitions and OSAC dispositions |
| `components` | array | Jira product components with readiness summaries |
| `evaluations` | array | Flat rule/component findings with targets and evidence |
| `mappingProblems` | array | Explicit ownership or structural mapping gaps |
| `diagnostics` | array | Collector-source health information |

Evaluation status is `pass`, `fail`, `error`, `unknown`, or `na`. An approved
exemption uses status `pass` and includes its approval record. `unknown` means
the requirement applies but trustworthy evidence is unavailable; `na` is
reserved for a requirement that does not apply to the target.

Readiness is cumulative: TP requires DP and GA requires DP plus TP. Each
component provides overall readiness and `complianceReadiness`, which excludes
Jira process-gate rules. The complete machine-readable contract and validation
rules live in `org-pulse-data/system-health/component-maturity/schemas/`.

The readiness score is `pass / (pass + fail + error + unknown)`; true `na`
evaluations are excluded. The UI also computes **Evidence coverage** from
applicable technical evaluations: `(pass + fail + error) / applicable
evaluations`. Unknown evidence is not credited by either measure and prevents
readiness.
Collector diagnostics include full messages for source health, missing
evaluators, and report-wide evidence coverage; an `ok` source means collection
succeeded, not that its maturity checks passed.

The demo fixture at
`fixtures/system-health/component-maturity/maturity-report.json` is generated
in offline mode and must not be treated as current OSAC maturity evidence.
